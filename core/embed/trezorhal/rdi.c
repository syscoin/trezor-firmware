...
Christophe Clavier, Jean-sébastien Coron, Nora Dabbous and Efficient Use of
...
void rdi_handler(uint32_t uw_tick) {
  if (rdi_disabled == secfalse) {  // if rdi enabled
    ...
    asm volatile(
        ...
        // loop ((delay // 3) + 2) times
        // two extra loops ensures that branch predictor learns the loop
        // every loop takes 3 ticks
        // r0 == (delay % 3) - 3
        "add r0, $3;"  // r0 += 3
        // r0 == delay % 3
        "and r0, r0, $3;"  // r0 %= 3, make sure that 0 <= r0 < 4
        "ldr r1, =table;"  // r1 = &table
        "tbb [r1, r0];"    // jump 2*r1[r0] bytes forward, that is goto wait_r0
        "base:"
        "b wait_2;"  // goto wait_2, make sure that we won't execute the table
        ...
        "wait_2:"
        "mov r0, r0;"  // wait one tick
        "wait_1:"
        "mov r0, r0;"  // wait one tick
        "wait_0:"
        :
        : "m"(delay)
        : "r0", "r1");  // wait (44 + delay) ticks
  } else {              // if rdi disabled or rdi_disabled corrupted
    ensure((rdi_disabled == sectrue) * sectrue, "Fault detected");
  }
}
